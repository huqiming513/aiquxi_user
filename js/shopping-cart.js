...
function addProduct(left, top, buynumber, price){
    ...
    flyer.fly({
        ...
        onEnd: function() {
            ...
            //购物车抖动
            $(".shopping-cart-wrap .shopping-cart imgs:first-child").animate({
                width: "4rem"
...
$(".prices-content-wrap, .taocan-wrap").on("click", ".price-box, .taocan-pic-cell, .curtain-content .small-bag, .curtain-content .big-bag, .small-bag-imgs, .big-bag-imgs", function(event) {
    ...
    if ($(this).find(".pic-box imgs").length > 0) {
        obj.image = $(this).find(".pic-box imgs").prop("src");
    }
    if ($(this).find(".taocan-pic-cell imgs").length) {
        obj.image = $(this).find(".taocan-pic-cell imgs").prop("src");
    }
    // 洗窗帘图片
    if ($(this).find(".small-bag imgs").length) {
        obj.image = $(this).find(".small-bag imgs").prop("src");
    }
    if ($(this).find(".big-bag imgs").length) {
        obj.image = $(this).find(".big-bag imgs").prop("src");
    }
    // 按袋洗图片
    if ($(this).find(".small-bag-imgs imgs").length) {
        obj.image = $(this).find(".small-bag-imgs imgs").prop("src");
    }
    if ($(this).find(".big-bag-imgs imgs").length) {
        obj.image = $(this).find(".big-bag-imgs imgs").prop("src");
    }
